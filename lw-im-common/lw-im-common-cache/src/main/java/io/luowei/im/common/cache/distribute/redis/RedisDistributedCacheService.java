package io.luowei.im.common.cache.distribute.redis;

import cn.hutool.core.util.StrUtil;
import cn.hutool.json.JSONUtil;
import io.luowei.im.common.cache.distribute.DistributedCacheService;
import io.luowei.im.common.cache.distribute.data.RedisData;
import io.luowei.im.common.cache.lock.DistributedLock;
import io.luowei.im.common.cache.lock.factory.DistributedLockFactory;
import io.luowei.im.common.cache.threadpool.ThreadPoolUtils;
...
/**
 * 基于Redis实现的分布式缓存，在满足分布式缓存的需求时，解决了缓存击穿、穿透和雪崩的问题
 * author: luowei
 * date:
 */
public class RedisDistributedCacheService implements DistributedCacheService {
    ...
    public <R, ID> R queryWithPassThrough(String keyPrefix, ID id, Class<R> type, Function<ID, R> dbFallback, Long timeout, TimeUnit unit) {
        ...
        if (r == null) {
            redisTemplate.opsForValue().set(key,EMPTY_VALUE,CACHE_NULL_TTL,unit);
        }
